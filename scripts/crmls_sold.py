...
if token:
    ...
    params = {
        ...
        "$filter": (
            f"MlsStatus eq 'Closed' and CloseDate ge {datetime(2026, 2, 1).isoformat(timespec='milliseconds')}Z "
            f"and CloseDate lt {datetime(2026, 3, 1).isoformat(timespec='milliseconds')}Z"
        ),
        "$top": 1000,
    }
    ...
    csv_file = output_dir / "CRMLSSold202602.csv"
